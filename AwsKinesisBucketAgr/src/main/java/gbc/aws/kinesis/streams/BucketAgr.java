...
public class BucketAgr {
    ...
    private static DataStream<TurnXAgrXProd> createSourceFromStaticConfig(StreamExecutionEnvironment env) {
        ...
        return env.addSource(new FlinkKinesisConsumer<>(inputStreamName, new ProjectSchema<>(TurnXAgrXProd.class), inputProperties));
    }
    ...
    private static FlinkKinesisProducer<Bucket> createSinkFromStaticConfig() {
        ...
        FlinkKinesisProducer<Bucket> sink = new FlinkKinesisProducer<Bucket>(new ProjectSchema<>(Bucket.class), outputProperties);
    ...
    KeyedProcessFunction<Integer, TurnXAgrXProd, Bucket> {
		...
		public void open(Configuration conf) {
		    MapStateDescriptor<Integer, Double> sumDesc =
		            new MapStateDescriptor<>("sumOfTransaction", Integer.class, Double.class);
